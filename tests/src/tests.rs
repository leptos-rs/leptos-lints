...
/// run if the documentation comment is updated.
#[test]
fn lints_readmes_are_updated() {
    let lints_dir = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("lints");
    let lints_dir_candidates =
        std::fs::read_dir(lints_dir).expect("Failed to read lints directory");

    let mut updated = vec![];
    for candidate in lints_dir_candidates {
        if let Ok(maybe_lint_dir) = candidate {
            let path = maybe_lint_dir.path();
            if !path.is_dir() {
                continue;
            }
            let lint_name = path
            ...
            let doc_comment =
                extract_lint_lib_doc_comment(&lint_lib_rs_content).unwrap_or_else(|| {
                    panic!(
                        "Failed to extract documentation comment from lib/{lint_name}/src/lib.rs",
                    )
                });
